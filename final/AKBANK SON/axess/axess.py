...
# --- 1. AYARLAR (v28 - Akıllı Sürüm) ---
BASE_URL = "https://www.axess.com.tr"
API_LIST_URL = "https://www.axess.com.tr/ajax/kampanya-ajax.aspx"
REFERER_URL = "https://www.axess.com.tr/kampanyalar"
DETAIL_SELECTOR = ".cmsContent.clearfix"
JSON_FILE_NAME = "axess_kampanyalar_v28.json" # <--- YENİ ÇIKTI DOSYASI
IMPORT_SOURCE_NAME = "Axess (Akbank)"
...
# --- 2. YARDIMCI FONKSİYONLAR (v28 DÜZELTMELERİ) ---
...
def extract_merchant_v25(title):
    try:
        match = re.search(r"Axess(?:’e|'e) özel\s+([^\s’']+)(?:’|'|’da|’de|'de|'da)", title, re.IGNORECASE)
...
def extract_participation_v28(text, title):
    """
    v28 GÜNCELLEMESİ (Hataları düzeltir):
    - Hata (ID 44, 15): Artık hem 'Juzdan' hem 'SMS'i arar ve birleştirir.
    - Hata (ID 78): Uzun katılım metinlerini (FATURAC...) kısaltmak yerine doğrudan atar.
    """
    part_points_list = []
    ...
    # KURAL (ID 78): Fatura kampanyası için özel kısaltma
    if "fatura talimat" in title.lower():
        try:
            ...
            if sms_match or kanallar:
                ...
                part_points_list = [short_text]
                method_category = short_text # v27 Düzeltmesi: Kategoriyi değil, metnin kendisini ata
    ...
    if match_sms:
        match_sms_text = temizle_metin(match_sms.group(0))
    ...
    # Eğer metin kategoriden daha açıklayıcıysa, metni kullan
    if len(part_points_list) == 1 and len(part_points_list[0]) > 35:
         method_category = part_points_list[0]
    # Eğer 2 katılım (SMS+App) bulunduysa, kategori yerine metinleri birleştir
...
def extract_eligible_customers_v28(text):
    """
    v28 GÜNCELLEMESİ (Hataları düzeltir):
    - Hata (ID 77): "dahil değildir" kuralını arar ve ÖNCESİNDEKİ
      "sadece ... yararlanabilir" kuralını uygular.
    - Hata (ID 26): "TROY" kuralını arar.
    - DİKKAT: ID 10 (dahil + dahil değil) hatası hala devam edebilir.
    """
    try:
        # KURAL 1 (ID 77): "sadece" kuralı (En spesifik)
        match_specific_include = re.search(
            r'Kampanyadan\s+sadece\s+(.+?)\s+(?:yararlanabilir|faydalanabilir)',
            text, re.IGNORECASE
        )
        if match_specific_include:
            customers_text = temizle_metin(match_specific_include.group(1))
            final_list_cleaned = clean_kart_list_v25(customers_text)
            if final_list_cleaned:
                return final_list_cleaned # Örn: ["Axess Öğrenci"]

        # KURAL 2 (ID 26): "TROY" kuralı
        match_troy = re.search(
            r'((?:Axess TROY|Akbank Kart TROY|Free TROY)[\w\s,]*?)(?:ile\s+bu\s+kartlar|dahildir|dâhildir|geçerlidir)', 
            text, 
            re.IGNORECASE
        )
        if match_troy:
            customers_text = temizle_metin(match_troy.group(1))
            final_list_cleaned = clean_kart_list_v25(customers_text)
            if final_list_cleaned:
                return final_list_cleaned # Örn: ["Axess TROY", "Akbank Kart TROY", "Free TROY"]

        # KURAL 3 (ID 107): Standart "dahildir" kuralı
        match_inclusion = re.search(
            r'Kampanyaya\s+((?:[\w\s,]*)(?:Axess|Wings|Free|Ticari|Bank’O Card Axess|Akbank Kart)[\w\s,]*?)(?:ile\s+bu\s+kartlar|dahildir|dâhildir)', 
            text, 
            re.IGNORECASE
        )
        if match_inclusion:
            customers_text = temizle_metin(match_inclusion.group(1))
            final_list_cleaned = clean_kart_list_v25(customers_text)
            
            # KURAL 4 (ID 10): "dahil değildir" istisnası (Basit)
            # Eğer "Bank'O Card ... dahil değildir" derse, listeden çıkar
            if re.search(r"Bank’O Card Axess.*?(?:dahil değildir|dâhil olmadığını|dâhil değildir)", text, re.IGNORECASE):
                if "Bank’O Card Axess" in final_list_cleaned:
                    final_list_cleaned.remove("Bank’O Card Axess")
    ...
    return ["Axess", "Free", "Wings", "Ticari", "Bank'O Card Axess"] # Default
...
def get_campaign_list(session):
    ...
    page_number = 1
    while True:
        params = { 'checkBox': '[0]', 'searchWord': '""', 'page': page_number }
        print(f"Sayfa {page_number} taranıyor...")
        try:
            response = session.get(API_LIST_URL, headers=HEADERS, params=params, timeout=20)
            ...
            if not found_new and page_number > 1:
                print("Yeni kampanya bulunamadı. Durduruluyor.")
                break
            page_number += 1
            time.sleep(random.uniform(0.5, 1.0))
        except requests.exceptions.RequestException as e:
            print(f"API isteğinde hata (Sayfa {page_number}): {e}")
            break
    print(f"Toplam {len(campaign_urls)} kampanya URL'si bulundu.")
    return campaign_urls

def get_campaign_details_arayanbuluyo_schema(session, url, campaign_id):
    print(f"Detaylar çekiliyor (ID: {campaign_id}): {url.split('/')[-1][:50]}...")
    try:
        response = session.get(url, headers=HEADERS, timeout=(10, 20)) 
        response.raise_for_status()
        soup = BeautifulSoup(response.content, 'html.parser')
    except requests.exceptions.RequestException as e:
        print(f"  [HATA] Detay sayfası yüklenemedi (ID: {campaign_id}): {e}")
        return None
    try:
        title_element = soup.find('h2', class_='pageTitle')
        title = title_element.get_text(strip=True) if title_element else "Başlık Bulunamadı"
        image_element = soup.select_one('.campaingDetailImage img')
        ...
        return None 
    try:
        details_element = soup.select_one(DETAIL_SELECTOR)
        full_kosullar_text = ""
        description = title 
        ...
        if details_element:
            ...
            if desc_element:
                description = temizle_metin(desc_element.get_text())
    ...
    try:
        min_spend_int, earning_str, discount_str, discount_perc_int, max_discount_int = \
            extract_financials_v27(description + " " + full_kosullar_text) # v27 (akıllı)
    except Exception as e:
    ...
    try:
        participation_method_str, participation_points_list = \
            extract_participation_v28(full_kosullar_text, title) # v28 (en akıllı)
    ...
    try:
        eligible_customers_list = extract_eligible_customers_v28(full_kosullar_text) # v28 (en akıllı)
    except Exception as e:
        print(f"  [UYARI] Müşteri listesi ayrıştırılamadı (ID: {campaign_id}): {e}")
        eligible_customers_list = ["Axess", "Free", "Wings", "Ticari", "Bank'O Card Axess"]
    try:
        merchant_str = extract_merchant_v25(title)
    ...
    if not merchant_str and valid_locations_list:
        try:
            ...
            print(f"  [UYARI] Merchant zenginleştirme hatası (ID: {campaign_id}): {e}")
...
if __name__ == "__main__":
    print(f"Axess Scraper (v28 - En Akıllı Sürüm) başlıyor...")
    start_time = time.time()
    with requests.Session() as session:
        all_campaign_urls = get_campaign_list(session)
        if all_campaign_urls:
            print(f"\n--- API'den {len(all_campaign_urls)} adet kampanya bulundu. Zengin detaylar çekiliyor... ---\n")
            all_campaign_data = []
            id_counter = 1 
            for url in all_campaign_urls:
                data = get_campaign_details_arayanbuluyo_schema(session, url, id_counter)
                if data:
