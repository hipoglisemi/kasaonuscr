...
# --- 1. AYARLAR (Free v28 - Akıllı Sürüm) ---
BASE_URL = "https://www.kartfree.com"
API_LIST_URL = "https://www.kartfree.com/ajax/kampanya-ajax-free.aspx"
REFERER_URL = "https://www.kartfree.com/free/kampanya/8/395/kampanyalar"
JSON_FILE_NAME = "axess_free_v28.json" # <--- YENİ ÇIKTI DOSYASI
IMPORT_SOURCE_NAME = "Axess Free (Akbank)"
...
# --- Seçiciler (HTML'den doğrulandı, Axess ile aynı) ---
...
def extract_merchant_v25_free(title): # <--- 'Free' için güncellendi
    try:
        match = re.search(r"Free(?:’ye|'ye) özel\s+([^\s’']+)(?:’|'|’da|’de|'de|'da)", title, re.IGNORECASE) # 'Axess' -> 'Free'
...
def extract_participation_v28(text, title):
    ...
    - Hata (ID 1, 10, 11, 12, 14, 15): Artık hem 'Juzdan' hem 'SMS'i arar ve birleştirir.
    - Hata (ID 21): Uzun katılım metinlerini (FATURAC...) kısaltmak yerine doğrudan atar.
    """
    part_points_list = []
    ...
    # KURAL (ID 21): Fatura kampanyası için özel kısaltma
    if "fatura talimat" in title.lower():
        try:
            ...
            if sms_match or kanallar:
                ...
                method_category = short_text # v27 Düzeltmesi
                return method_category, part_points_list
        except Exception: pass 
    
    # --- DÜZELTME (ID 1, 10, 11, 12, 14, 15) ---
    match_app_text = ""
    match_sms_text = ""
...
def extract_eligible_customers_v28_free(text): # <--- 'Free' için güncellendi
    """
    v28 GÜNCELLEMESİ (Hataları düzeltir):
    - Hata (ID 7, 10): "dahil değildir" istisnalarını arar.
    - Hata (ID 2, 3): Standart "dahildir" kuralını arar.
    - Varsayılanı sadece ["Free"] yapar.
    """
    try:
        # KURAL 1: "sadece" kuralı (En spesifik)
        ...
        if match_inclusion:
            ...
            # KURAL 3 (ID 7, 10): "dahil değildir" istisnası
    ...
    return ["Free"] # Varsayılanı "Free" yap
...
def get_campaign_list(session):
    ...
    page_number = 1
    while True:
        params = { 'checkBox': '[]', 'searchWord': '""', 'page': page_number } 
        ...
        try:
            ...
            links = soup.select(LIST_ITEM_SELECTOR) 
            if not links:
...
def get_campaign_details_arayanbuluyo_schema(session, url, campaign_id):
    ...
    try:
        ...
        return None 
    try:
        details_element = soup.select_one(DETAIL_CONTAINER_SELECTOR)
        full_kosullar_text = ""
        description = title 
    ...
    # --- Veri Çıkarım Fonksiyonları (v28 - En Akıllı) ---
    ...
    participation_method_str, participation_points_list = \
        extract_participation_v28(full_kosullar_text, title) # <--- v28
    difficulty_level_str = map_difficulty_level_v25(participation_method_str)
    eligible_customers_list = extract_eligible_customers_v28_free(full_kosullar_text) # <--- v28 (Free)
    merchant_str = extract_merchant_v25_free(title) # <--- (Free)
...
if __name__ == "__main__":
    print(f"Axess Free Scraper (v28 - Akıllı Sürüm) başlıyor...")
