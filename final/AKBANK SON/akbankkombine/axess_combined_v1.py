...
# --- 1. AYARLAR (Nihai Birleşik Sürüm) ---
...
BANK_CONFIGS = [
    {
        ...
        "api_params": { 'checkBox': '[0]', 'searchWord': '""' }, # Axess filtresi
        ...
        "image_base_url": "https://www.axess.com.tr" # Resimler buradan çekilecek
    },
    {
        "name": "Axess Free (Akbank)",
        ...
        "api_params": { 'checkBox': '[]', 'searchWord': '""' }, # Free filtresi
        "list_item_selector": ".campaingBox a.dLink",
        "detail_container_selector": ".cmsContent.clearfix",
        "title_selector": "h2.pageTitle",
        "image_selector": ".campaingDetailImage img",
        "image_base_url": "https://www.axess.com.tr" # Kırık resim olmasın diye Axess'ten çek
    },
    {
        "name": "Axess Business (Ticari)", # <--- YENİ EKLENDİ
        "base_url": "https://www.axess.com.tr", # Ana site 'axess.com.tr'
        ...
        "api_params": { 'checkBox': '[]', 'searchWord': '""' }, # <--- SENİN BULDUĞUN
        "list_item_selector": ".campaingBox a.dLink",
        "detail_container_selector": ".cmsContent.clearfix",
        "title_selector": "h2.pageTitle",
        "image_selector": ".campaingDetailImage img",
        "image_base_url": "https://www.axess.com.tr"
    }
]

JSON_FILE_NAME = "tum_akbank_kampanyalari_final.json" # <--- TEK ÇIKTI DOSYASI
...
# --- YENİ WRAPPER FONKSİYONLAR ---
# Bu fonksiyonlar, 'Axess', 'Free' veya 'Business' olmasına göre doğru alt fonksiyonu çağırır

def extract_merchant_wrapper(title, provider_name):
    if "Free" in provider_name:
        try:
            match = re.search(r"Free(?:’ye|'ye) özel\s+([^\s’']+)(?:’|'|’da|’de|'de|'da)", title, re.IGNORECASE)
        ...
        return None
    else: # Varsayılan Axess / Business
        try:
...
def extract_eligible_customers_wrapper(text, provider_name):
    try:
        ...
        # KURAL 3 (Standart "dahildir" kuralı)
        ...
        if match_inclusion:
            ...
            # KURAL 4 (ID 10): "dahil değildir" istisnası
    ...
    if "Free" in provider_name:
        return ["Free"]
    elif "Business" in provider_name:
        return ["Axess Business"]
    else:
        return ["Axess", "Free", "Wings", "Ticari", "Bank'O Card Axess"]
...
def get_campaign_details_arayanbuluyo_schema(session, url, bank_config):
    ...
    try:
        ...
        print(f"  [HATA] Detay sayfası yüklenemedi: {e}")
        return None
    try:
        ...
        print(f"  [HATA] Başlık/Görsel alınamadı: {e}")
        return None 
    try:
    ...
    # ID YOK: Supabase'in otomatik ataması için 'id' alanı kaldırıldı.
    kampanya_objesi = {
...
if __name__ == "__main__":
    ...
    all_campaign_data = [] # Tüm bankaların verisi burada toplanacak
    
    with requests.Session() as session:
        
        # Yapılandırmadaki her banka için döngü başlat
        for bank_config in BANK_CONFIGS:
            ...
            if all_campaign_urls:
                ...
                for url in all_campaign_urls:
                    ...
                    # Sunucuyu yormamak için rastgele bekle
