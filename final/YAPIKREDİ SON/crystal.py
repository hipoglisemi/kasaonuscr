...
# --- 1. AYARLAR (v5.5.1 - Crystal Card / Kart Adı Fix) ---
...
REFERER_URL = "https://www.crystalcard.com.tr/kampanyalar"
JSON_FILE_NAME = "crystal_v5.5.1_Nihai.json" 
IMPORT_SOURCE_NAME = "Crystal Card (Yapı Kredi)" # Provider kısmında bu görünecek (Doğru)
...
def temizle_metin(text):
    """v5.4.5 - JSON-safe text cleaner (Türkçe tırnak fix)"""
    if not text:
        return ""
    try:
        text = str(text)
...
def format_tarih_iso_v25(tarih_str, is_end_date=False):
    ...
    try:
        ...
        if is_end_date: iso_str = f"{yil_str}-{ay_num.zfill(2)}-{gun_str}T23:59:59Z"
        else: iso_str = f"{yil_str}-{ay_num.zfill(2)}-{gun_str}T00:00:00Z"
...
def extract_dates_from_text_v25(text):
    ...
    try:
        match = re.search(r'(\d{1,2}\.\d{1,2}\.\d{4})\s*(?:-|–)\s*(\d{1,2}\.\d{1,2}\.\d{4})', text, re.IGNORECASE)
        if match:
...
def extract_financials_v5_3_play_adapted(text):
    ...
    all_worldpuan_vals = []
    pattern_worldpuan = re.findall(r'(\d+[\d.,]*)\s*' + tl_suffix + r'\s*(?:puan|hediye|iade|değerinde)', finance_text, re.IGNORECASE)
    if not pattern_worldpuan: pattern_worldpuan = re.findall(r'(\d+[\d.,]*)\s*' + tl_suffix + r'.{0,15}?(?:puan|hediye|iade|değerinde)', finance_text, re.IGNORECASE)
    ...
    if all_worldpuan_vals: best_worldpuan_val = max(all_worldpuan_vals); worldpuan_str = f"{format_rakam(best_worldpuan_val)} TL Puan"
    taksit_numbers = []
    ...
    try:
        if discount_perc_int is not None and max_discount_int is not None and not spend_calculated:
            if indirim_str and indirim_str.startswith("%"): 
                min_spend_int = int(max_discount_int / (discount_perc_int / 100)) 
    ...
    try:
        ...
        if pattern_opet and not spend_calculated:
            ...
            if earn_per_tx > 0:
                ...
                min_spend_int = int((num_tx * spend_per_tx) + trigger_spend)
                spend_calculated = True
    except Exception: pass
    try:
        ...
        if pattern_ek and not spend_calculated:
            ...
            if total_earn_per_tx > 0:
                ...
                min_spend_int = int(num_tx * spend_per_tx) 
                spend_calculated = True
    except Exception: pass
    try:
...
def extract_merchant_v25_play_adapted(title):
    try:
        match = re.search(r"Crystal(?:’e|'e) özel\s+([^\s’']+)(?:’|'|’da|’de|'de|'da)", title, re.IGNORECASE)
        if match: return temizle_metin(match.group(1))
        match = re.search(r"([^\s’']+)(?:’|'|’da|’de|'de|'da)\s+(?:peşin|özel|indirim|worldpuan|taksit|puan)", title, re.IGNORECASE)
        if match: return temizle_metin(match.group(1))
    except Exception: pass
    return None
...
def clean_kart_list_v5_4_2(kart_listesi):
    ...
    for item in final_list_intermediate:
        ...
        if 'kredi kart' not in item.lower() and 'ticari kart' not in item.lower():
            item = re.sub(r'kredi', '', item.strip(), flags=re.IGNORECASE)
        item = re.sub(r'kartlar\u0131$', 'kartları', item.strip(), flags=re.IGNORECASE) 
        item = re.sub(r'kartlar$', 'kartlar', item.strip(), flags=re.IGNORECASE) 
        item = re.sub(r'kart\u0131$', 'kartı', item.strip(), flags=re.IGNORECASE) 
        ...
        item = re.sub(r'\s+için$', '', item.strip(), flags=re.IGNORECASE)
        item = re.sub(r'\s+ile$', '', item.strip(), flags=re.IGNORECASE)
        item = re.sub(r'[()]', '', item)
        item = item.strip()
...
def extract_eligible_customers_v5_4_2_play_adapted(text):
    try:
        match_specific_include = re.search(
            r'(?:Kampanyadan|Kampanyaya|Kampanya)\s+(?:sadece|yaln\u0131zca)\s+([\w\s,]*kart[\w\s,]*?)\s+(?:için\s+geçerlidir|d\u00e2hildir|dahildir|ge\u00e7erlidir|yararlanabilir|faydalanabilir)',
            text, re.IGNORECASE
        )
        if match_specific_include:
            customers_text = match_specific_include.group(1)
            final_list_cleaned = clean_kart_list_v5_4_2(customers_text) 
            if final_list_cleaned:
                return final_list_cleaned 
        match_inclusion = re.search(
            r'Kampanyaya\s+((?:[\w\s,]*)(?:Adios|World|Yapı Kredi|Bireysel|TLcard|Albaraka|Anadolubank|Vakıfbank|ticari)[\w\s,]*?kart[\w\s,]*?)(?:ile\s+bu\s+kartlar|dahildir|dâhildir)', 
            text, 
            re.IGNORECASE
        )
        if match_inclusion:
        ...
        if match_payment_cards:
            ...
            if final_list_cleaned:
                return final_list_cleaned
        match_valid_cards = re.search(
            r'Kampanya\s+([\w\s,]*kart[\w\s,]*?)\s+(?:geçerlidir|geçerli olacaktır)',
            text, re.IGNORECASE
        )
        if match_valid_cards:
            customers_text = match_valid_cards.group(1)
            final_list_cleaned = clean_kart_list_v5_4_2(customers_text)
            if final_list_cleaned:
                return final_list_cleaned
    except Exception:
    ...
    # v5.5.1 Düzeltmesi: Varsayılan kart adı "Crystal Card" (Yapı Kredi yok)
    return ["Crystal Card"] 
...
def get_campaign_list_from_api(session):
    print("Aşama 1: Crystal API'den kampanya listesi çekiliyor (Sayfalandırma başlıyor)...")
    ...
    for item in all_campaigns_from_api:
        try:
            ...
            if url not in all_campaign_url_map:
                all_campaign_url_map[url] = (image_url, api_title) 
                
        except Exception as e:
            print(f"❌ HATA 1 (API Yapı Hatası): API listesi işlenemedi: {e}")

    print(f"Toplam {len(all_campaign_url_map)} benzersiz kampanya URL'si bulundu.")
    return all_campaign_url_map


def get_campaign_details_snake_case_schema(session, url, image_url, api_title, campaign_id):
    
    for attempt in range(MAX_RETRIES):
        print(f"   [{campaign_id}] Çekiliyor (Deneme: {attempt + 1}/{MAX_RETRIES}): {url.split('/')[-1][:40]}...")
        try:
    ...
    try:
        # --- CRYSTAL'A ÖZEL HTML AYRIŞTIRMA (v1.1'den alındı) ---
        all_conditions_list = []
        full_kosullar_text = ""
        conditions_list = ["Detaylar Bulunamadı"]
        valid_from_tr, valid_until_tr = None, None
        
        main_content_div = soup.select_one('.campaign-terms') 
        
        if main_content_div:
            all_conditions_list = [temizle_metin(item.text) for item in main_content_div.select('ul > li, p') if item.text and len(item.text.strip()) > 10]
            conditions_list = clean_conditions_v25(all_conditions_list)
            full_kosullar_text = " ".join(all_conditions_list)

            date_heading = main_content_div.find('strong', string=re.compile(r'Kampanya tarihi', re.IGNORECASE))
            
            if date_heading:
                date_li = date_heading.find_next_sibling('ul').find('li')
                if date_li:
                    date_text = date_li.text 
                    valid_from_tr, valid_until_tr = extract_dates_from_text_v25(date_text)
            else:
                if all_conditions_list:
                    date_text_list = [li for li in all_conditions_list if re.search(r'tarihleri arasında|tarihine kadar', li, re.IGNORECASE)]
                    date_text = date_text_list[0] if date_text_list else all_conditions_list[0] 
                    valid_from_tr, valid_until_tr = extract_dates_from_text_v25(date_text)
        else:
            date_div = soup.select_one('.start-end-date p')
            if date_div:
                date_text = temizle_metin(date_div.text) 
                valid_from_tr, valid_until_tr = extract_dates_from_text_v25(date_text)
            
            content_div = soup.select_one('.sub-content')
            if content_div:
                h3_kosullar = content_div.find('h3', string=re.compile(r'Kampanya Koşulları', re.IGNORECASE))
                if h3_kosullar:
                    ul_kosullar = h3_kosullar.find_next_sibling('ul')
                    if ul_kosullar:
                        conditions_list = [temizle_metin(li.text) for li in ul_kosullar.select('li') if li.text and len(li.text.strip()) > 10]
                        full_kosullar_text = " ".join(conditions_list)
    ...
    # --- v5.5.1 Akıllı Fonksiyonlarını Çağırma ---
    ...
    try:
        if not valid_from_tr and not valid_until_tr:
    ...
    try:
        eligible_customers_list = extract_eligible_customers_v5_4_2_play_adapted(full_kosullar_text)
    except Exception as e:
        print(f"  [UYARI] Müşteri listesi ayrıştırılamadı (ID: {campaign_id}): {e}")
        eligible_customers_list = ["Crystal Card"] 
...
# --- 4. ANA ÇALIŞTIRMA KISMI ---

if __name__ == "__main__":
    print(f"Crystal Scraper (v5.5.1 - Kart Adı Fix) ba\u015fl\u0131yor...")
        ...
        if all_campaign_url_map:
            ...
            for url, (image_url, api_title) in all_campaign_url_map.items():
                try:
                    ...
                    print(f"  [KRİTİK PARSE HATASI] ID {id_counter} ({url}) işlenemedi: {e}")
            ...
            if all_campaign_data:
                try:
                    ...
                    print("\n" + "="*50)
                    print(f"🎉 BAŞARILI! (v5.5.1 - Crystal Stabil Sürüm) 🎉")
