...
# --- 1. AYARLAR (v5.5.1 - World Card / Nihai Stabil Sürüm) ---
# World Card API yapısı Adios/Play'den parametre olarak biraz farklıdır, güncel URL:
LIST_API_URL = "https://www.worldcard.com.tr/api/campaigns?campaignSectorKey=tum-kampanyalar&campaignSectorId=6d897e71-1849-43a3-a64f-62840e8c0442&campaignTypeId=0&keyword="
BASE_URL = "https://www.worldcard.com.tr"
REFERER_URL = "https://www.worldcard.com.tr/kampanyalar"
JSON_FILE_NAME = "world_v5.5.1_Nihai.json" 
IMPORT_SOURCE_NAME = "World Card (Yapı Kredi)" # Provider'da bu görünecek
...
# --- 2. YARDIMCI FONKSİYONLAR (Evrensel v5.5.1 Standardı) ---
...
def temizle_metin(text):
    ...
    try:
        text = str(text)
        # Türkçe/tipografik tırnakları düz tırnaklara çevir
        text = text.replace("“", '"').replace("”", '"') 
        text = text.replace("‘", "'").replace("’", "'") 
        text = text.replace("„", '"').replace("‚", "'")
        
        # Geçersiz kontrol karakterlerini temizle (Whitelist)
        text = re.sub(r'[^\x20-\x7E\u00A0-\uFFFF]', ' ', text)
        
        # Tüm boşlukları tek boşluğa indirge
...
def extract_merchant_v25_world_adapted(title):
    """World Card için özel adaptasyon"""
    try:
        match = re.search(r"World(?:’e|'e|card'a)\s+özel\s+([^\s’']+)(?:’|'|’da|’de|'de|'da)", title, re.IGNORECASE)
...
def extract_eligible_customers_v5_4_2_world_adapted(text):
    """World Card için uyarlanmış müşteri çıkarma"""
    try:
        ...
        match_inclusion = re.search(
            r'Kampanyaya\s+((?:[\w\s,]*)(?:World|Worldcard|Yapı Kredi|Bireysel|TLcard|Albaraka|Anadolubank|Vakıfbank|ticari)[\w\s,]*?kart[\w\s,]*?)(?:ile\s+bu\s+kartlar|dahildir|dâhildir)', 
            text, 
            re.IGNORECASE
        )
        if match_inclusion:
    ...
    # v5.5.1 Düzeltmesi: Varsayılan kart adı "World Card"
    return ["World Card"] 
...
def get_campaign_list_from_api(session):
    print("Aşama 1: World Card API'den kampanya listesi çekiliyor (Sayfalandırma başlıyor)...")
...
def get_campaign_details_snake_case_schema(session, url, image_url, api_title, campaign_id):
    ...
    try:
        eligible_customers_list = extract_eligible_customers_v5_4_2_world_adapted(full_kosullar_text)
    except Exception as e:
        print(f"  [UYARI] Müşteri listesi ayrıştırılamadı (ID: {campaign_id}): {e}")
        eligible_customers_list = ["World Card"] 

    try:
        merchant_str = extract_merchant_v25_world_adapted(title)
...
if __name__ == "__main__":
    print(f"World Card Scraper (v5.5.1 - Nihai Stabil Sürüm) ba\u015fl\u0131yor...")
        ...
        if all_campaign_url_map:
            ...
            if all_campaign_data:
                try:
                    ...
                    print("\n" + "="*50)
                    print(f"🎉 BAŞARILI! (v5.5.1 - World Card Stabil Sürüm) 🎉")
