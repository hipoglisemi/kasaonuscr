...
# --- 1. AYARLAR (v5.5.1 - Adios Card / Kart Adı Fix) ---
...
REFERER_URL = "https://www.adioscard.com.tr/kampanyalar"
JSON_FILE_NAME = "adios_v5.5.1_Nihai.json" 
IMPORT_SOURCE_NAME = "Adios Card (Yapı Kredi)" # Provider kısmında bu görünecek (Doğru)
...
def extract_merchant_v25_play_adapted(title):
    try:
        match = re.search(r"Adios(?:’e|'e) özel\s+([^\s’']+)(?:’|'|’da|’de|'de|'da)", title, re.IGNORECASE)
...
def extract_eligible_customers_v5_4_2_play_adapted(text):
    ...
    # v5.5.1 Düzeltmesi: Varsayılan kart adı "Adios Card" (Yapı Kredi yok)
    return ["Adios Card"] 
...
def get_campaign_list_from_api(session):
    print("Aşama 1: Adios API'den kampanya listesi çekiliyor (Sayfalandırma başlıyor)...")
...
def get_campaign_details_snake_case_schema(session, url, image_url, api_title, campaign_id):
    ...
    try:
        ...
        if details_element:
            ...
            print(f"  [UYARI] '.campaign-terms' bulunamadı (ID: {campaign_id}). Sayfa gövdesi kullanılacak.")
    ...
    # --- v5.19 Akıllı Fonksiyonlarını Çağırma ---
    ...
    try:
        ...
        eligible_customers_list = ["Adios Card"] # Fallback düzeltildi
...
if __name__ == "__main__":
    print(f"Adios Scraper (v5.5.1 - Evrensel Stabil Sürüm) ba\u015fl\u0131yor...")
        ...
        if all_campaign_url_map:
            ...
            if all_campaign_data:
                try:
                    ...
                    print("\n" + "="*50)
                    print(f"🎉 BAŞARILI! (v5.5.1 - Adios Stabil Sürüm) 🎉")
